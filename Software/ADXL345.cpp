...
bool ADXL345::begin()
{
	...
	correction_offset.XAxis=0;
	correction_offset.YAxis=0;
	correction_offset.ZAxis=0;

	correction_range.XAxis=1;
	correction_range.YAxis=1;
	correction_range.ZAxis=1;
...
// Read normalized values with proper tare
Vector ADXL345::readNormalizeCorrected(float gravityFactor){
    Vector v = readNormalize(gravityFactor);
    v.XAxis = (v.XAxis-correction_offset.XAxis)*correction_range.XAxis;
    v.YAxis = (v.YAxis-correction_offset.YAxis)*correction_range.YAxis;
    v.ZAxis = (v.ZAxis-correction_offset.ZAxis)*correction_range.ZAxis;
...
void ADXL345::setNormalizeCorrectionXYZ(Vector *min_values, Vector *max_values){



	correction_offset.XAxis = (max_values->XAxis + min_values->XAxis)/2.0;
	correction_offset.YAxis = (max_values->XAxis + min_values->XAxis)/2.0;
	correction_offset.ZAxis = (max_values->XAxis + min_values->XAxis)/2.0;

	//let's suppose that now Ymin = -Ymx
	correction_range.XAxis = ADXL345_GRAVITY_EARTH/(max_values->XAxis-correction_offset.XAxis);
	correction_range.YAxis = ADXL345_GRAVITY_EARTH/(max_values->YAxis-correction_offset.YAxis);
	correction_range.ZAxis = ADXL345_GRAVITY_EARTH/(max_values->ZAxis-correction_offset.ZAxis);
}
